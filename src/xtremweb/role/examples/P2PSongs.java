...
public class P2PSongs {
     ...
     * download files from the P2P network members.
     * 
     ...
     * 
     * @code [ "the" ,
     * ("THE cardigans-my favorite game.mp3","THE doors-riders on THE storm.mp3"
     ...
     * part of the other type of entries, each of them are put together with a
     ...
     * 
     * @code [ 9efe0c2a047d66685dfdc0ffffa92446 , ( 234.34.56.71 , 27.23.78.99 ,
     * 134.240.123.25 )].
     * 
     ...
     * network members. For this purpose they run two instances of bitdew: first
     * instance has a DR to manage the HTTP repository to store your files, a DC
     * to locally index their data once somebody wants to download a file, and a
     * DT to manage the file transfers. The second bitdew instance references
     ...
     * there</li>
     * 
     * <li> DLPT library is here, please download it and move it at the same
     * level than the file bitdew-stand-alone-XXX.jar</li>
     * 
     * <li> Download the file <a
     * href="http://www.bitdew.net/properties_p2p.json">properties_p2p.json</a>
     * </li>
     * 
     * <li> Execute the following command
     * 
     * @code pc-1$ java -cp sbam_standalone.jar:bitdew-stand-alone-X.X.X.jar
     * xtremweb.role.cmdline.CommandLineTool serv --file properties_p2p.json dc
     * 
     * @endcode
     * 
     ...
     * of bitdew-stand-alone-X.X.X.jar, the DLPT jar file, and the file <a
     * href="http://www.bitdew.net/properties_p2p.json"
     * >properties_p2p.json</a></li>
     * 
     * Let's look at three important JSON objects in this file: <ol> <li>
     * <b>xtremweb.serv.dc : {ddc: false} </b> </li> In this object you can turn
     * on/off the DHT, as it is a demanding process, it is turn off by default,
     * to turn on just set the property ddc (distributed data catalog ) to true
     * <li> <b>xtremweb.core.http: {port:
     * 8080,path:"/",documentRoot:".",uploadServlet:"/fileupload",p2pServlet:
     * "/p2pquery",bootstrapNode: "address.to.my.host.com"}</b><br> The
     * bitdew-stand-alone jar comes with an embedded JETTY web server, JSON
     * object <b>xtremweb.core.http</b>, contains different properties needed to
     * configure it, for this case you just need to configure path and
     * bootstrapNode. </li>
     * 
     * <ul>
     * 
     * <li><b>path:</b> root for static resources relative to BITDEW_HOME, in
     * this case, our root will be BITDEW_HOME </li>
     * 
     * <li><b>bootstrapNode:</b> the node where the DHT is running, if you erase
     * this property, the default will be localhost</li>
     * 
     * </ul>
     * 
     * <li> <b>xtremweb.serv.dr.http:{name: "http",
     * port:"8080",path:"songs"}</b><br>
     * 
     * This JSON object describes to bitdew the http repository where files will
     * be stored. To make your files available, you can change "path" for the
     * directory where songs are stored, in our case we have them on
     * $BITDEW_HOME/songs (remember that <em>path</em> value is relative to the
     * BITDEW_HOME variable) </li> </ol>
     * 
     * <li> Put properties-p2p.json file in the same level that
     * bitdew-stand-alone-X.X.X.jar, and execute :
     * 
     * @code pc-2$ java -cp sbam_standalone.jar:bitdew-stand-alone-X.X.X.jar
     * xtremweb.role.examples.CommandLineTool serv --file properties_p2p.json dc
     * dr dt ds
     * 
     * @endcode
     * 
     ...
     * 
     * @code pc-2$ java -cp sbam_standalone.jar:bitdew-stand-alone-X.X.X.jar
     * xtremweb.role.examples.P2PSongs properties-p2p.json <bootstrap_node>
     * <directory_to_share>
     * 
     * @endcode The following commands will: <ul> <li>Start locally the differnt
     * dX's services</li> <li>Create two Bitdew API's (first one referencing the
     * locally created dc and dt, second one referencing the distributed hash
     * table on the \<bootstrap node\> </li> <li>Register the songs contained in
     * the local \<directory_to_share\> folder in the P2P network </li> </ul>
     * 
     * </ul>
     * 
     * <b>(Commented and detailed code is shown below, if you want exact details
     * of how this things are actually performed).</b> <li> You can repeat steps
     * 2,3 in pc-3, and in as many computers as you desire, each computer will
     * be a peer of our application </li> <h2>Song searching </h2> <li> Now, we
     * can request for files on the P2P network: from any host in your network
     * open your webbrowser and type http://localhost:8080/findterm.html You
     * should see a window similar to the next one : \image html findterm.png In
     * this window you need to fill three fields : <ul> <li>Target directory :
	...
	log.info("i must be able to see this ");
	try {
	    //
	    ...
	    //
	    log.info("connecting re;ote bitdez");
	    bitdewddc = new BitDew(ddc, dr, dt, ds, true);
	    log.info("connecting local bitdew");
     ...
     */
    public void put(String dirpath) throws Exception {
	try {
	    File dir = new File(dirpath);
	    if (!dir.exists()) {
		throw new FileNotFoundException("The directory " + dirpath + " does not exists");
	    }
