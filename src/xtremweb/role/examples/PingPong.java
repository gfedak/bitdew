...
/**
 * <code>PingPong</code> shows how to create and delete Data and how to react to
 * data schedule and deletion
 * 
...
public class PingPong {
	 ...
	 * 
	 * @param host
	 *            <code>String</code>, the hostName of the dc, dr and ds server
	 * @param port
	 *            <code>int</code>, port on which the server hould be contacted
	 * @exception Exception
	 *                if an error occurs
	 */
	...
	/**
	 * <code>Ping</code> create a Data Ping and schedule it. Then waits for the
	 * Pong data and delete it.
	 * 
	 * @exception Exception
	 *                if an error occurs
	 */
	public void Ping() throws Exception {
		...
		Attribute attr = activeData
				.createAttribute("{name: 'PingPong',replicat : -1 }");
		...
		log
				.info(" data "
						...
						+ " ["
						+ ping.getuid()
						+ "] is successfully created and scheduled with attribute PingAttr = {replicat = -1 } ");
	...
	public class PingCallback implements ActiveDataCallback {
		 ...
		 * @param attr
		 *            an <code>Attribute</code> value
		 */
		public void onDataScheduled(Data data, Attribute attr) {
			log.info("Received data: " + data.getname() + ", whose uid is "
					+ data.getuid() + ", and with Attribute " + attr.getname());
			try {
		...
		public void onDataDeleted(Data data, Attribute attr) {
			log.info("Data : " + data.getname()
					+ " has been deleted, whose uid is " + data.getuid());// +
																			// ", and with Attribute "
																			// +
																			// attr.getname());
	...
	/**
	 * <code>Pong</code> waits for the Ping Data. When Ping is received, it
	 * creates and schedule Pong and delete Ping,
	 * 
	 */
	public void Pong() {
		// register the PongCallback to handle Data creation
		activeData.registerActiveDataCallback(new PongCallback(bitdew,
				activeData));
		activeData.start();
	}
	...
	public class PongCallback implements ActiveDataCallback {
		 ...
		 * 
		 * @param bd
		 *            a <code>BitDew</code> value
		 * @param ad
		 *            an <code>ActiveData</code> value
		 */
			...
			try {
				...
				log
						.info(" data "
								...
								+ " ["
								+ pong.getuid()
								+ "] is successfully created and scheduled with attribute "
								+ attr.getname());
	...
	static public void usage() {
		System.out
				.println("Usage : java xtremweb.role.examples.PingPong ping|pong [hostName] [port]");
		System.exit(0);
	}
