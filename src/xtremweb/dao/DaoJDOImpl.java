...
/**
 * This Dao implementation uses jdo to perform the operations
 * declared on the interface (see InterfaceDao )
 * @author jsaray
 *
 */
public class DaoJDOImpl implements InterfaceDao {
	...
	try {
	    ...
	    log
		    .debug("No Database configuratioin found for DBInterfaceFactory : "
			    + ce);
	...
	properties.setProperty("javax.jdo.option.ConnectionUserName", mainprop
		.getProperty("xtremweb.core.db.user", "sa"));
	properties.setProperty("javax.jdo.option.ConnectionPassword", mainprop
		.getProperty("xtremweb.core.db.password", ""));
	...
	if (mainprop.getProperty("xtremweb.core.db.connectionPooling") != null) {
	    properties.setProperty("org.jpox.connectionPoolingType", mainprop
		    .getProperty("xtremweb.core.db.connectionPooling"));
     ...
     * @param clazz the class to get the name
     * @param name the name
