...
import xtremweb.serv.dt.jsaga.xml.XMLInterface;
...
public class JsagaTransfer extends BlockingOOBTransferImpl {
	...
	xml = new XmlDomImpl(mainprops.getProperty("xtremweb.serv.dr.jsaga.type"),mainprops.getProperty("xtremweb.serv.dr.jsaga.contextFilePath"));
	ctx = ContextFactory.createContext();

	// Define the voms server
	ctx.setAttribute(Context.SERVER, xml.getElement("Server"));

	// Define your VO
	ctx.setAttribute(Context.USERVO, xml.getElement("UserVO"));

	// Define your proxy life time
	ctx.setAttribute(Context.LIFETIME, xml.getElement("LifeTime"));
	...
	ctx.setAttribute(Context.USERCERT, xml.getElement("UserCert"));

	// The path of your user key
	ctx.setAttribute(Context.USERKEY,  xml.getElement("UserKey"));

	// The password of your user key
	ctx.setAttribute(Context.USERPASS, xml.getElement("UserPass"));

	// A directory containing the CA certificates
	// (http://dist.eugridpma.info/distribution/igtf/current/accredited/tgz/)
	ctx.setAttribute(Context.CERTREPOSITORY, xml.getElement("CertRepository"));

	// Needed ??? Might be leaved empty
	ctx.setAttribute(VOMSContext.VOMSDIR, xml.getElement("VomsDir"));
