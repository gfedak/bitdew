...
public class TransferManager {
	...
	public OOBTransfer createOOBTransfer(Transfer t,DaoTransfer daocheck) throws OOBException {
		Data d = null;
		Locator rl = null;
		Locator ll = null;
		Protocol lp = null;
	...
	private void checkTransfer() {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				log.debug("Checking Transfer : " + trans.getuid() + ":"
	 ...
	 */
	public boolean isTransferComplete(Data data) {//José Francisco : I have to build another dao here because it is not taking into account
		//the commits of daocheck this is not a sensful solution but i need to keep going.
		...
		try {
			...
			if (results == null) {
				...
				while (iter.hasNext()) {
					Transfer trans = (Transfer) iter.next();
					log.debug("scanning transfer locator " + trans.getlocatorlocal()+" tuid: "+ trans.getuid() + " "
							+ trans.getdatauid() + " status : "+trans.getstatus());
					if (trans.getstatus() != TransferStatus.TODELETE)
						return false;
				}
			}
